...
export default function SearchBar(props: { onChange: (txt: string) => void }) {
  ...
  return (
    <View style={{ backgroundColor: Colors.lightgray, alignItems: "center", borderRadius: 17, flexDirection: "row", paddingHorizontal: 17 }}>
      <Feather name="search" style={{ fontSize: RF(16) }} />
      <TextInput value={q} onBlur={hideCancel} onFocus={showCancel} ref={tiref} onChangeText={handleChange} placeholderTextColor={Colors.placeholder} placeholder="Search" style={{ marginLeft: 6, fontSize: RF(16), flex: 1, paddingVertical: 10 }} />
      {q.length > 0 && <TouchableOpacity style={{ paddingHorizontal: 5, paddingVertical: 10 }} onPress={() => handleChange("")}><Feather style={{ fontSize: RF(16) }} name="x" /></TouchableOpacity>}
